...
├── _intake/                    # Mobile notes landing zone (from Obsidian)
...
**Mobile → PC** : Notes written in Obsidian's `00_Inbox` folder are pulled into `AI-Cockpit/_intake/`. The AI agent automatically checks this folder at session start and processes your raw input.
...
- **[Migration Roadmap](docs/MIGRATION_ROADMAP.md)** — Strategy for moving to an autonomous OS powered by OpenClaw.
...
├── _intake/                    # モバイルメモの受信箱 (Obsidian から)
...
マイグレーションの戦略については [docs/MIGRATION_ROADMAP.md](docs/MIGRATION_ROADMAP.md) をご覧ください。
...
- **モバイル → PC**: Obsidian の `00_Inbox` フォルダに書いたメモが `AI-Cockpit/_intake/` に取り込まれ、AIエージェントがセッション開始時に自動処理。
